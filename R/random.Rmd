...
```{r}
dataset2 <- Edu[,c(11,83,186)]
dataset2$JTEMODE = factor(dataset2$JTEMODE,
                       labels = c(1:10))
test_set <- formalsetEdu[,c(8,10)]
test_set$JTEMODE = factor(test_set$JTEMODE,
                       labels = c(1:10))
finalerr <- 100
idealtrees = 0
...
ggplot(Edu,aes(x=Edu$AgeGroup_RW,y=Edu$workDuration))+geom_boxplot() + coord_cartesian(ylim=c(100,400))
ggplot(formalsetEdu,aes(x=formalsetEdu$AgeGroup,y=formalsetEdu$workDuration))+geom_boxplot()
