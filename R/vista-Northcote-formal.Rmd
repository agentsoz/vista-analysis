...
```{r, fig.width=15, fig.height=10}
ggplot(Wrk,aes(x=Wrk$AgeGroup_RW,fill=Wrk$JTWMODE)) + geom_bar(position = "fill")
ggplot(finalset,aes(x=formalsetWork$AgeGroup,fill=formalsetWork$JTWMODE)) + geom_bar(position = "fill")
```
...
line_reg = randomForest(x=dataset2[-4],y=dataset2$STARTIME,ntree = 100,weights = Wrk$CW_ADJTWWGT_LGA)
...
```{r}
summary(lm(Wrk$STARTIME~Wrk$SEX + Wrk$AGE + Wrk$MAINACT))

ggplot(Wrk,aes(x=Wrk$AgeGroup_RW,y=Wrk$STARTIME))+geom_boxplot() + coord_cartesian(ylim = c(460,670))
...
ggplot(Wrk,aes(x=Wrk$JTWMODE, y=Wrk$JTWDIST))+geom_boxplot() + coord_cartesian(y=c(0,100))
ggplot(formalsetWork,aes(x=formalsetWork$JTWMODE, y=formalsetWork$JTWDIST))+geom_boxplot() + coord_cartesian(y=c(0,100))
```
